...
import getBookingInventory from '@salesforce/apex/BikeParkingGraphicalViewController.getBookingInventory';
import { FlowAttributeChangeEvent, FlowNavigationNextEvent } from 'lightning/flowSupport';

export default class bikeParkingGraphicalViewBooking extends LightningElement {
...
@wire(getBookingInventory, { projectId: '$recordId'})
...
filterData() {
    ...
    this.propertyFilters.forEach(propFilter => {
        ...
        else if (propFilter.name === 'status') {
            console.log('status');
            this.filteredPropertyGraph.towers.forEach(tower => {
                tower.floors.forEach(floor => {
                    let filteredProperties = floor.properties.filter(
                        filteredproperty => !propFilter.filterValues.includes(filteredproperty.status)
                    );
