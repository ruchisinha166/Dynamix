...
import getInventory from '@salesforce/apex/PropertyGraphicalViewController.getInventory';
import { FlowAttributeChangeEvent, FlowNavigationNextEvent } from 'lightning/flowSupport';

export default class PropertyGraphicalView extends LightningElement {
    ...
    filterData() {
        ...
        this.propertyFilters.forEach(propFilter => {
            ...
            else if (propFilter.name === 'unitType') {
                this.filteredPropertyGraph.towers.forEach(tower => {
                    tower.floors.forEach(floor => {
                        let filteredProperties = floor.properties.filter(
                            filteredproperty => !propFilter.filterValues.includes(filteredproperty.unitType)
                        );
